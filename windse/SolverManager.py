...
class SteadySolver(GenericSolver):
    ...
    def Solve(self):
        ...
        ### try to compute the integrals of things
        dg0 = FunctionSpace(self.problem.dom.mesh, "DG", 0) # create a P0 function space
        nuT_elem_int_form = self.problem.nu_T
        nuT_elem_int = project(nuT_elem_int_form, dg0, solver_type="cg", preconditioner_type="jacobi")
        nuT_elem_int_file = self.params.Save(nuT_elem_int,"nuT_elem_int",subfolder="solutions/",val=self.iter_val) # DEBUG!!!!! later, move into Save (see below)
        V_elem_form = CellVolume(self.problem.dom.mesh)/Constant(self.problem.dom.volume)
        V_elem = project(V_elem_form, dg0, solver_type="cg", preconditioner_type="jacobi")
        V_elem_file = self.params.Save(V_elem,"V_elem",subfolder="solutions/",val=self.iter_val) # DEBUG!!!!! later, move into Save (see below)
        inuT_elem_int_form = self.problem.nu_T*CellVolume(self.problem.dom.mesh)
        inuT_elem_int = project(inuT_elem_int_form, dg0, solver_type="cg", preconditioner_type="jacobi")
        inuT_elem_int_file = self.params.Save(inuT_elem_int,"inuT_elem_int",subfolder="solutions/",val=self.iter_val) # DEBUG!!!!! later, move into Save (see below)
